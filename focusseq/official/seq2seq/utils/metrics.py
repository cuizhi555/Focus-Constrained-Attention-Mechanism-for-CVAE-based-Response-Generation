...
def word_padded_cross_entropy_loss(logits, labels, smoothing, vocab_size):
  ...
  with tf.name_scope("loss", values=[logits, labels]):
    ...
    with tf.name_scope("smoothing_cross_entropy", values=[logits, labels]):
      ...
      labels = tf.sets.set_intersection(labels, labels) 
      ...
      mask = tf.ones_like(weights, dtype=tf.float32)
      mask = tf.pad(mask, [[0, 0], [3, 0]])[:, :-3]
      targets = weights * mask
    
      on_value = tf.to_float(tf.equal(targets, 1.0))
      off_value = tf.to_float(tf.equal(targets, 0.0))
      
      soft_targets = on_value + off_value
      ...
      #soft_targets = tf.reduce_sum(soft_targets, axis=1)
      #logits = tf.expand_dims(logits, axis=1)
      #length = tf.shape(labels)[1]
      #logits = tf.tile(logits, [1, length, 1])
      ...
      positive_weight = tf.expand_dims(tf.reduce_sum(tf.to_float(tf.equal(targets, 0.0)), axis=1) / float(vocab_size), -1)
      negative_weight = tf.expand_dims(tf.reduce_sum(tf.to_float(tf.equal(targets, 1.0)), axis=1) / float(vocab_size), -1)
      total_weight = positive_weight * on_value + negative_weight * off_value

    #weights = tf.to_float(tf.not_equal(labels, 0))

    return xentropy, total_weight
