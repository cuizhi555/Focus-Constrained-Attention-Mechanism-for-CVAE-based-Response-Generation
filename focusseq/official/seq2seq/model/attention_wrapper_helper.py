...
class _BaseAttentionMechanism(AttentionMechanism):
  ...
  def __init__(self,
    ...
    focus_vector = array_ops.expand_dims(focus_vector, -1)
    _keys_tmp = array_ops.concat([memory, focus_vector], axis=-1)

    with ops.name_scope(
        name, "BaseAttentionMechanismInit", [nest.flatten(memory), nest.flatten(_keys_tmp)]):
      ...
      _keys_tmp = _prepare_memory(
          _keys_tmp, memory_sequence_length,
          check_inner_dims_defined=check_inner_dims_defined)
      ...
      self._keys = (
          self.memory_layer(_keys_tmp) if self.memory_layer  # pylint: disable=not-callable
          else self._values)
...
def _bahdanau_score(processed_query, keys, history, normalize):
  ...
  if normalize:
    ...
    return math_ops.reduce_sum(
        normed_v * math_ops.tanh(keys + processed_query + b + history), [2])
  else:
    return math_ops.reduce_sum(v * math_ops.tanh(keys + processed_query + history), [2])
...
class BahdanauAttention(_BaseAttentionMechanism):
  ...
  def __init__(self,
    ...
    self._name = name
    self.history_layer = layers_core.Dense(num_units, dtype=dtype, name="history_dense")

  def __call__(self, query, state, history):
    """Score the query based on the keys and values.
    ...
    with variable_scope.variable_scope(None, "bahdanau_attention", [query]):
      ...
      history = array_ops.expand_dims(history, -1)
      processed_history = self.history_layer(history)
      score = _bahdanau_score(processed_query, self._keys, processed_history, self._normalize)
    alignments = self._probability_fn(score, state)
    next_state = alignments
    return alignments, next_state
...
def _compute_attention(attention_mechanism, cell_output, attention_state, 
                       history, attention_layer):
  """Computes the attention and alignments for a given attention_mechanism."""
  
  alignments, next_attention_state = attention_mechanism(
      cell_output, state=attention_state, history=history)
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
  ...
  def call(self, inputs, state):
    ...
    for i, attention_mechanism in enumerate(self._attention_mechanisms):
      ...
      history_vector = tf.cond(tf.greater(previous_alignment_history[i].size(), 0), 
          lambda : math_ops.reduce_sum(previous_alignment_history[i].stack(), axis=0),
          lambda : array_ops.zeros([cell_batch_size, self.encoder_sequence_length], dtype=inputs.dtype))

      attention, alignments, next_attention_state = _compute_attention(
          attention_mechanism, cell_output, previous_attention_state[i], history_vector,
