...
class BeamSearchIgnoreUnkDecoder(decoder.Decoder):
  ...
  def __init__(self,
    ...
    self._start_inputs = array_ops.zeros_like(self._start_inputs)   
...
#unk_token = ops.convert_to_tensor(
#    0, dtype=dtypes.int32, name="unk_token")
...
def _beam_search_step(time, logits, next_cell_state, beam_state, batch_size,
  ...
  #ignore unk token when decoding
  rest_logits_1, unk_logits, rest_logits_2 = array_ops.split(logits, [UNK_ID, 1, logits_shape[-1] - 3], -1)
  unk_logits = unk_logits + NEG_INF
  new_logits = array_ops.concat([rest_logits_1, unk_logits, rest_logits_2], -1)
  new_logits = array_ops.reshape(new_logits, logits_shape)
  

  step_log_probs = nn_ops.log_softmax(new_logits)
